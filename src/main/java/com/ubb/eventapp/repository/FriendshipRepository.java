...
public interface FriendshipRepository extends JpaRepository<Friendship, FriendshipId> {
     ...
     */
    @Query("select f from Friendship f where (f.user2.id = :userId) and f.estado = :estado")
    List<Friendship> findByUserIdAndEstado(@Param("userId") Long userId, @Param("estado") FriendshipState estado);
}
